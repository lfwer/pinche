...
$(document).ready(function() {
	...
	$("button[name='timeLimitC']").click(
			function() {
				...
				$("button[name='timeLimitC']").removeClass(
		...
		$.post(server.path+"/pinkerInfo/addPinkerInfoSubmit",$form.serialize(),"json").done(function(result) {
			...
			if (result && result.valid == true) {
				...
				$("#viewDiv").slideDown(500,function() {
					$("#viewDiv").load(lfwer.rootName+'/pages/pinkerInfo/views/'+result.message+'.html?type=index', function() {
						$(".mask").hide();
					});
				});
			} else {
